...
import Base: hash, ==, print, show_spec_linfo
...
function print_flat(io::IO, lilist::Vector{LineInfo}, n::Vector{Int}, combine::Bool, cols::Integer, sortedby)
    ...
    for i = 1:length(n)
        li = lilist[i]
        print(io, lpad(string(n[i]), wcounts, " "), " ")
        print(io, rpad(rtruncto(li.file, wfile), wfile, " "), " ")
        print(io, lpad(string(li.line), wline, " "), " ")
        fname = li.func
        if !li.fromC && !isnull(li.outer_linfo)
            fname = sprint(show_spec_linfo, Symbol(li.func), get(li.outer_linfo))
        end
        print(io, rpad(ltruncto(fname, wfunc), wfunc, " "))
        println(io)
    end
end
